...
class GurobiOptimizer:
    ...
    def optimiser_taux(self, client: ClientProfile, pret: PretDemande) -> Dict:
        ...
        try:
            ...
            self.model = gp.Model("Tarification_Optimale")
            self.model.setParam('OutputFlag', 0)  # Pas de sortie console
            
            # Variables de décision
            r = self.model.addVar(lb=r_min, ub=r_max, name="taux_interet")
            
            # On utilise D = D_0 * [1 + epsilon * (r_bar - r)] directement dans l'objectif
            # Pas besoin de variable pour D
            
            # 6. Contrainte ratio d'endettement
            # Calcul de la mensualité exacte
            # M = V * r_m * (1 + r_m)^n / ((1 + r_m)^n - 1)
            r_m = r / 12  # Taux mensuel
            n = pret.duree * 12  # Nombre de mois
            
            # Approximation pour Gurobi (linéaire)
            # mensualite_approx = (pret.montant * r) / (12 * pret.duree)
            # self.model.addConstr(
            #     mensualite_approx + client.charges_mensuelles <= R_max * client.revenu_mensuel,
            #     "ratio_endettement"
            # )
            
            # Alternative: utilisation de contrainte linéaire simplifiée
            ...
            # Approximation: mensualité ≈ prêt * taux / (durée * 12)
            self.model.addConstr(
                (pret.montant * r) / (pret.duree * 12) <= mensualite_max,
                "ratio_endettement_approx"
            )
            
            # 7. Fonction objectif : Maximiser profit
            # Profit = D * [V * k * (r - c_ref) - c_op - V * PD * LGD]
            # Où D = D_0 * [1 + epsilon * (r_bar - r)]
            
            profit_unitaire = (
                pret.montant * pret.duree * (r - c_ref) -  # Marge d'intérêt
                c_op -                                      # Coûts opérationnels
                pret.montant * PD * LGD                    # Perte attendue
            )
            ...
            # Définir objectif
            self.model.setObjective(profit_total, GRB.MAXIMIZE)
